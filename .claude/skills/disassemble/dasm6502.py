#!/usr/bin/env python3
"""Disassemble 6502 machine code from main.bin.

Usage:
    python .claude/skills/disassemble/dasm6502.py START [END]

    START: hex address (e.g. 7780 or $7780 or 0x7780)
    END:   hex address (exclusive), defaults to next RTS/JMP/BRK + 1

Reads main.bin (loaded at $0500) and prints a disassembly listing.
"""
...
def disassemble(data: bytes, start: int, end: int) -> None:
...
def main() -> None:
    if len(sys.argv) < 2:
        ...
        print(f'  START/END are hex addresses (e.g. 7780, $7780, 0x7780)')
        sys.exit(1)
        ...
        while i < len(data):
            ...
            if b in IMPLIED:
                if b in TERMINAL:
                    ...
                    break
                i += 1
            elif b in IMMEDIATE or b in ZEROPAGE or b in ZEROPAGE_X or b in ZEROPAGE_Y or b in INDIRECT_X or b in INDIRECT_Y or b in BRANCH:
                i += 2
