...
### Step 1: Get the disassembly from Ghidra

Use `mcp__ghidra-server__disassemble_function` to get the instruction listing. Also use `mcp__ghidra-server__get_function_by_address` to get the address range.

If the address is not a known function in Ghidra, use `mcp__ghidra-server__read_memory` to read the raw bytes and disassemble manually. Look for an RTS ($60), JMP ($4C), or other terminal instruction to find the end.
